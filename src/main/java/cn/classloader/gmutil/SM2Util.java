...
public class SM2Util extends GMBaseUtil {
     ...
     * @return
     */
    public static String getPrivateKey(AsymmetricKeyParameter key) {
     ...
     * @return
     */
    public static String getPublicKey(AsymmetricKeyParameter key) {
        ECPublicKeyParameters publicKey = (ECPublicKeyParameters) key;
     ...
     * @return
     */
    public static ECPublicKeyParameters getPublicKeyParameters(String publicKey) {
        final byte[] key = Hex.decode(publicKey);
        final ECCurve curve = DOMAIN_PARAMS.getCurve();
        return new ECPublicKeyParameters(curve.decodePoint(key), DOMAIN_PARAMS);
    }

    public static ECPublicKeyParameters getPublicKeyParameters(byte[] publicKey) {
        final ECCurve curve = DOMAIN_PARAMS.getCurve();
        return new ECPublicKeyParameters(curve.decodePoint(publicKey), DOMAIN_PARAMS);
    }
     ...
     * @return
     */
    public static ECPrivateKeyParameters getPrivateKeyParameters(String privateKey) {
        final BigInteger key = BigIntegers.fromUnsignedByteArray(Hex.decode(privateKey));
        return new ECPrivateKeyParameters(key, DOMAIN_PARAMS);
    }
