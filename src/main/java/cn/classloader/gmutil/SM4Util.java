...
public class SM4Util extends GMBaseUtil {
    ...
    private static final String SM4_CHARACTERS = "qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM0123456789";

    public static String generateNuccKey() {
        return generateNuccKey(DEFAULT_KEY_SIZE);
    }

    public static String generateNuccKey(int keySize) {
        if (keySize % 8 != 0) {
            throw new RuntimeException("sm4密钥长度必须为8的倍数");
        }
        StringBuilder sb = new StringBuilder(16);
        Random random = new Random();
        for (int i = 0; i < keySize / 8; i++) {
            int index = random.nextInt(SM4_CHARACTERS.length());
            sb.append(SM4_CHARACTERS.charAt(index));
        }
        return sb.toString();
    }
